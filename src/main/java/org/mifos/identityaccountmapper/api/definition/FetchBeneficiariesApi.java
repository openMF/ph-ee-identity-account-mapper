...
public interface FetchBeneficiariesApi {
    ...
    ResponseEntity<Page<FetchBeneficiariesResponseDTO>> fetchAllBeneficiary(@RequestHeader(value = "X-Registering-Institution-ID") String registeringInstitutionId,  @RequestParam(value = "page", required = false, defaultValue = "0") Integer page,
                                                                            @RequestParam(value = "pageSize", required = false, defaultValue = "20") Integer pageSize) throws ExecutionException, InterruptedException;
}
