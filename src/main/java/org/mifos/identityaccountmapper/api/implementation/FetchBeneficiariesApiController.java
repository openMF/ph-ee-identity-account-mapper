...
public class FetchBeneficiariesApiController implements FetchBeneficiariesApi {
    ...
    @Override
    public ResponseEntity<FetchBeneficiariesResponseDTO> fetchBeneficiary( String payeeIdentity, String registeringInstitutionId) throws ExecutionException, InterruptedException {
        if (StringUtils.isBlank(registeringInstitutionId)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        }
        FetchBeneficiariesResponseDTO fetchBeneficiariesResponseDTO = fetchBeneficiariesService.fetchBeneficiary(payeeIdentity, registeringInstitutionId);
        return ResponseEntity.status(HttpStatus.OK).body(fetchBeneficiariesResponseDTO);
    }

    @Override
    public ResponseEntity<Page<FetchBeneficiariesResponseDTO>> fetchAllBeneficiary( String registeringInstitutionId, Integer page, Integer pageSize) throws ExecutionException, InterruptedException {
        if (StringUtils.isBlank(registeringInstitutionId)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        }
        Page<FetchBeneficiariesResponseDTO> fetchBeneficiariesResponseDTO = fetchBeneficiariesService.fetchAllBeneficiaries(page, pageSize, registeringInstitutionId);
        return ResponseEntity.status(HttpStatus.OK).body(fetchBeneficiariesResponseDTO);
    }
}
